...
def execute_intent(
    ...
    if stored_intent.get("status") == "payment_verified":
        return {
            ...
            "payment": stored_intent.get("payment"),
            "idempotent": True
        }

    # Reuse the existing Razorpay order when checkout is being retried.
    if stored_intent.get("status") == "payment_pending" and stored_intent.get("payment"):
        return {
            "intent_id": intent_id,
            "status": "payment_pending",
            "payment": stored_intent["payment"],
            "idempotent": True
        }
    ...
    stored_intent["execution_count"] = stored_intent.get("execution_count", 0) + 1
...
def verify_payment_endpoint(
    ...
    intent_id = next(
        (
            ...
            if stored.get("payment", {}).get("order_id") == request.razorpay_order_id
        ),
        None
    )
    ...
    if intent_id is None:
        raise HTTPException(
            status_code=404,
            detail="Razorpay order is not associated with a known purchase intent"
        )
    ...
    # Never expose or log the signature. Keep it only server-side for audit/idempotency.
    stored_intent["payment"]["signature_verified"] = True
    stored_intent["payment"]["_signature"] = request.razorpay_signature
    ...
    audit_logs.append(
        AuditEvent(
            ...
            status="pending",
            reason="Signature verified, but payment is not captured yet"
        )
    )
...
async def razorpay_webhook(
    ...
    if x_razorpay_event_id and x_razorpay_event_id in processed_webhook_events:
        return {
            "status": "ignored",
    ...
    if x_razorpay_event_id:
        processed_webhook_events.add(x_razorpay_event_id)
    ...
    # Reject stale webhook replays. Razorpay events contain created_at as Unix time.
    created_at = payload.get("created_at")
    if isinstance(created_at, (int, float)) and abs(time.time() - created_at) > 300:
        raise HTTPException(
            status_code=400,
    ...
    payment_id = entity.get("id") if entity.get("entity") == "payment" else None

    intent_id = next(
        (
            current_id
            for current_id, stored in intents.items()
            if stored.get("payment", {}).get("order_id") == order_id
        ),
        None
    )
    ...
    if event_name == "payment.captured":
        stored_intent["status"] = "payment_verified"
        stored_intent.setdefault("payment", {}).update({
            "status": "payment_verified",
            "payment_id": payment_id or stored_intent.get("payment", {}).get("payment_id"),
            "captured": True,
        ...
        stored_intent["status"] = "payment_verified"
        stored_intent.setdefault("payment", {})["webhook_confirmed"] = True
        ...
        audit_logs.append(
            AuditEvent(
                ...
                event="order_paid",
                status="success",
                reason="Razorpay order.paid webhook received"
            )
        )
...
def get_intent_status(
    ...
    return {
        "intent_id": intent_id,
        "status": stored.get("status", "intent_created"),
        "approved": stored.get("approved", False),
        "policy": stored.get("policy"),
        "payment": {
            key: value
            for key, value in (stored.get("payment") or {}).items()
            if not key.startswith("_")
        },
        "execution_count": stored.get("execution_count", 0),
